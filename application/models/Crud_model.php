...
class Crud_model extends CI_Model {
	...
	/*ticket ttacking*/
	function ticketTracking($obj){ 
		$sql["ticket_number"] 	= 	$obj->ticket_number;
		$sql["approved_by"] 	= 	$obj->approved_by;
		$sql["approval_status"] = 	$obj->approval_status;
		$sql["comment"] 		= 	$obj->comment;
		$sql["approved_date"] 	=  	date('Y-m-d H:i:s');
		$sql["status"] 			=  	$obj->status; 
		$this->db->insert('ticket_tracking',$sql);
	}
	function update_room($obj)
    {
        //	update room info
        $this->db->where('room_id', $obj->room_id);
        //
        if(!empty($obj->date_in)) {
            $this->db->set('water_old_date', $obj->date_in);
            $this->db->set('elect_old_date', $obj->date_in);
        }
        if(!empty($obj->old_water)){
            $this->db->set('water_old_usage', $obj->old_water);
        }
        if(!empty($obj->old_elect)){
            $this->db->set('elect_old_usage', $obj->old_elect);

        }
        //
        if(!empty($obj->booking)) {
            $this->db->set('booking', $obj->booking);
            $this->db->set('staying_id', $obj->staying_id);
        }
        $this->db->update('room');
    }
	...
	// get description
	function get_system_settings_description($price_for) {
        return $this->db->get_where('settings',array('type'=>$price_for))->row()->description;
    }
